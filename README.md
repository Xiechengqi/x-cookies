...
3. 配置环境变量：
   ```bash
   export TWITTER_ACCOUNT="my_account"   # 必须，脚本只接受单账号
   export X_AUTH_TOKEN="xxxxx"           # 必须，用于自动登录
   export CDP_ENDPOINT="http://localhost:9222"  # 可选，默认即为该值
   ```
4. 运行脚本（脚本会在导出后自动调用 `twitter-scraper/main.go` 进行一次搜索测试）：
   ```bash
   python main.py
   ```
...
- `TWITTER_ACCOUNT` 用于决定输出文件名，脚本仅支持单账号。
...
- 自动测试阶段可通过 `SCRAPER_TEST_QUERY`、`SCRAPER_TEST_COUNT`、`TWITTER_SCRAPER_PROXY` 调整搜索参数，设置 `SKIP_SCRAPER_TEST=true` 可跳过 Go 测试。
